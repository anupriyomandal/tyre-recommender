...
class ResponseGenerator:
    ...
    def generate_stream(self, query: str, vehicle_rows: list[dict], history: list[dict] | None = None):
        """
        Stream natural language recommendation from rows as SSE chunks.
        Yields HTML-safe strings that can be directly appended to the DOM.
        """
        if not client:
            raise ValueError("OpenAI API key is not configured.")

        # Short-circuit: no vehicle rows — conversational mode
        if not vehicle_rows:
            answer = self._generate_conversational(query, history)
            yield answer
            return
...
The 1.4 VTVT and 1.6 VTVT S use <b>185/65R15 SecuraDrive TL</b>, with an upsize option of <b>195/60R15 SecuraDrive TL</b>."""
        ...
        logger.info("Sending prompt to OpenAI (streaming)...")
        try:
            messages = [
                {
                    "role": "system",
    ...
    def _generate_conversational_stream(self, query: str, history: list[dict] | None):
        """Handle follow-up or conversational queries that have no new vehicle data."""
        if not history:
            yield "I can help with tyre recommendations. Please share your vehicle's make and model."
            return
